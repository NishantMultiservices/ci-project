...
$id = intval($_GET['id'] ?? 0);
$ht = $conn->query("SELECT * FROM hall_tickets WHERE id = $id AND is_active = 1")->fetch_assoc();

if (!$ht) {
    $page_title = 'Not Found';
    include '../includes/header.php';
    echo '<section class="section" style="padding-top:60px;text-align:center;"><div class="container"><h2>Hall Ticket Not Found</h2><p style="margin:16px 0;">This hall ticket does not exist or has been removed.</p><a href="index.php" class="btn btn-primary">← Back to Hall Tickets</a></div></section>';
    include '../includes/footer.php'; exit;
}

$page_title = htmlspecialchars($ht['title']) . ' - Hall Ticket';
include '../includes/header.php';

$posts = $conn->query("SELECT * FROM hall_ticket_posts WHERE hall_ticket_id = {$ht['id']} ORDER BY id");
...
.dt-hero::before{content:'';position:absolute;top:-50%;right:-8%;width:400px;height:400px;background:radial-gradient(circle,rgba(251,191,36,.1) 0%,transparent 60%)}
...
<div class="dt-bar">
    <div class="container">
        <div class="dt-bar-inner">
            <a href="index.php" class="dt-bar-back">← Back to Hall Tickets</a>
            <div class="dt-bar-actions" style="display:flex;gap:6px;">
                <?php if (isLoggedIn()): ?>
                <button onclick="toggleSave(this,'hall_ticket',<?php echo $ht['id']; ?>)" style="width:34px;height:34px;display:flex;align-items:center;justify-content:center;border-radius:8px;border:1px solid #eef0f2;background:white;cursor:pointer;font-size:1rem;color:#889096;">☆</button>
                <?php endif; ?>
            </div>
        </div>
    </div>
...
<section class="section" style="padding-top:0;">
    <div class="container" style="max-width:920px;">
        ...
        <div class="dt-hero">
            <h1><?php echo htmlspecialchars($ht['title']); ?></h1>
            <div class="dt-hero-org">🏛 <?php echo htmlspecialchars($ht['organization']); ?></div>
            <div class="dt-hero-meta">
                <div>📋 <?php echo htmlspecialchars($ht['exam_name']); ?></div>
                <div>📅 <?php echo date('d M Y', $examDate); ?></div>
                <?php if (!$isExpired): ?><div style="color:<?php echo $isUrgent?'#fbbf24':'rgba(255,255,255,.5)'; ?>;font-weight:600;">
                    <?php echo $isUrgent ? "🔥 $daysLeft day".($daysLeft>1?'s':'')." left" : '✅ Upcoming'; ?>
                </div><?php endif; ?>
            </div>
        </div>

        <?php if ($ht['description']): ?>
        <div class="dt-card">
            <div class="dt-card-hd"><div class="hd-icon" style="background:#eef2ff;color:#4f46e5;">📄</div><h3>Description</h3></div>
            <div class="dt-card-bd"><p><?php echo nl2br(htmlspecialchars($ht['description'])); ?></p></div>
        </div>
        <?php endif; ?>

        <?php if ($posts && $posts->num_rows > 0): ?>
        <div class="dt-card">
            <div class="dt-card-hd"><div class="hd-icon" style="background:#f3e8ff;color:#9333ea;">📋</div><h3>Exam Posts / Papers</h3></div>
            <div style="padding:0;">
                <table class="dt-tbl">
                    <thead><tr><th>Post / Paper</th><th>Time</th><th>Download</th></tr></thead>
                    <tbody><?php while($p=$posts->fetch_assoc()): ?>
                        <tr>
                            <td><strong><?php echo htmlspecialchars($p['post_name']); ?></strong></td>
                            <td><?php echo htmlspecialchars($p['exam_time'] ?: 'TBA'); ?></td>
        ...
        <div class="dt-card">
            <div class="dt-card-hd"><div class="hd-icon" style="background:#fef3c7;color:#d97706;">⚠️</div><h3>Instructions</h3></div>
            <div class="dt-card-bd"><p><?php echo nl2br(htmlspecialchars($ht['instructions'])); ?></p></div>
        </div>
        <?php endif; ?>
        ...
        <div class="dt-card">
            ...
            <div class="dt-card-bd">
                <?php if ($ht['download_url']): ?>
                    <a href="<?php echo htmlspecialchars($ht['download_url']); ?>" target="_blank" class="btn btn-primary">📥 Download Hall Ticket</a>
                <?php else: ?>
                    <p style="color:#94a3b8;">Download will be available closer to the exam date.</p>
                <?php endif; ?>
            </div>
        </div>
